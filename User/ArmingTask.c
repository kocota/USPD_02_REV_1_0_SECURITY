...
void ThreadArmingTask(void const * argument)
{
	...
	for(;;)
	{
		...
		{
			//----Контроль шлейфа №1--------------------------------------------------------------------------------------------------------------------------------
			if( (HAL_GPIO_ReadPin(GPIOH, GPIO_PIN_4) == GPIO_PIN_SET) && ( (control_registers.control_loop_reg&0x01) != 0x00 ) )
			{
				if(button_state_arm1<10){button_state_arm1++;}
				else if(button_state_arm1>=10){state_arm1=1;}

			}
			else if( (HAL_GPIO_ReadPin(GPIOH, GPIO_PIN_4) == GPIO_PIN_RESET) && ( (control_registers.control_loop_reg&0x01) != 0x00 ) )
			{
